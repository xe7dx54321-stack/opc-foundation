...
IMPLEMENTED_SOURCE_TYPES = {
    ...
    "conference_transcript",  # Phase 2C 新增
    "analyst_action",  # Phase 2D 新增
    "media_mention",  # Phase 2E 新增
}
...
def validate_research_config(config: ResearchArchiveConfig) -> list[str]:
    ...
    for idx, src in enumerate(config.sources):
        ...
        # conference_transcript：所有 profile 都需要 url 或 base_url
        if src.source_type == "conference_transcript":
            profile = src.extraction_profile or "conference_event_list"
            if not src.url and not src.base_url:
                errors.append(
                    f"source [{src.source_id}] conference_transcript ({profile}) 需要 url 或 base_url"
                )
        # analyst_action：所有 profile 都需要 url 或 base_url
        if src.source_type == "analyst_action":
            profile = src.extraction_profile or "analyst_action_table"
            if not src.url and not src.base_url:
                errors.append(
                    f"source [{src.source_id}] analyst_action ({profile}) 需要 url 或 base_url"
                )
        # media_mention：所有 profile 都需要 url 或 base_url
        if src.source_type == "media_mention":
            profile = src.extraction_profile or "media_mention_article_list"
            if not src.url and not src.base_url:
                errors.append(
                    f"source [{src.source_id}] media_mention ({profile}) 需要 url 或 base_url"
                )
